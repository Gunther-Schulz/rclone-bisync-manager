...
from daemon import daemon_main, stop_daemon, print_daemon_status
from sync import perform_sync_operations
from utils import check_tools, ensure_rclone_dir, handle_filter_changes
from logging_utils import log_message, log_error, ensure_log_file_path
...
def main():
    ...
    if args.command == 'daemon':
        ...
        from config import sync_jobs, specific_sync_jobs
        ...
        for key in paths_to_sync:
            if key in sync_jobs:
                ...
                log_error(f"Specified sync job '{key}' not found in configuration")
